...
[ ! -s $tmp  -o $time -lt 10 ] && /usr/bin/openstack-status 2>&1 |egrep -v 'disabled on boot|=|not sourced' >$tmp


get_status(){
        h_srv=`cat $log|grep -iw "^$1:"|awk '{print $NF}' 2>/dev/null`
	n_srv=`cat $tmp|grep -iw "^$1:"|awk '{print $NF}' 2>/dev/null`
