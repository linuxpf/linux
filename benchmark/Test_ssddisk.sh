...
for bs in 4k 64k 128k 1024k 2048k 4096k;do
    ...
    echo "### write bs=$bs bksize=$bksize count=$coun" >> $log
    echo 3 > /proc/sys/vm/drop_caches
    dd if=/dev/zero of=/data1/40gssd_${bs} bs=$bs count=$coun oflag=direct 2>>$log 
    sleep 3
    i=$((i+1))
done
...
for bs in 4k 64k 128k 1024k 2048k 4096k;do
    ...
    bksize=`echo $bs|sed 's/k//g'`
    coun=`echo $size/$bksize|bc`
    echo "###read bs=$bs count=$coun" >> $log
    echo 3 > /proc/sys/vm/drop_caches
    dd if=/data1/40gssd_${bs}  of=/dev/null bs=$bs count=$coun iflag=direct 2>>$log
